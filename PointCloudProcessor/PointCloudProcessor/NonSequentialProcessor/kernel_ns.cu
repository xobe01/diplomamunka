...
bool arePlanesIntersecting(Plane* plane1, Plane* plane2)
{
	...
	if ((plane1->normal - plane2->normal).length() < normalTreshold &&
		(abs(Vec3<double>::dot_product(plane1->normal, plane2->furthestNormalPoints.first - plane1->planePointPos)) < distanceTreshold ||
			abs(Vec3<double>::dot_product(plane1->normal, plane2->furthestNormalPoints.second - plane1->planePointPos)) < distanceTreshold ||
			abs(Vec3<double>::dot_product(plane2->normal, plane1->furthestNormalPoints.first - plane2->planePointPos)) < distanceTreshold ||
			abs(Vec3<double>::dot_product(plane2->normal, plane1->furthestNormalPoints.second - plane2->planePointPos)) < distanceTreshold)) 
	{
		for (int k = 0; k < plane1->edges.size(); k++) {
			changeBaseTo2D(plane1->edges[k], { plane1->normal, plane1->pointDirections.first });
		}
		for (int k = 0; k < plane2->edges.size(); k++) {
			changeBaseTo2D(plane2->edges[k], { plane1->normal, plane1->pointDirections.first });
		}
		for (int k = 0; k < plane1->edges.size(); k++) {
		...
		for (int k = 0; k < plane2->edges.size(); k++) {
			changeBaseTo2D(plane2->edges[k], { plane2->normal, plane2->pointDirections.first });
		}
	}
	return false;
}
			...
			while (allPlanes[j]->edges.size() > 0) {
				for (size_t i = 1; i < allPlanes[j]->edges.size(); i++) {
					if (allPlanes[j]->edges[0]->isHole == allPlanes[j]->edges[i]->isHole) 
					{
						...
						createdNewPolygon = true;
						mergePolygons(allPlanes[j], allPlanes[j]->edges[0], allPlanes[j]->edges[i], allPlanes[j]->edges[0]->isHole ? holeNeighbours : nonHoleNeighbours, innerCreatedEdges,
							createdNewPolygon);
